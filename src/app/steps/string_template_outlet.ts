...
export class NzStringTemplateOutletDirective implements OnChanges {
  ...
  private shouldRecreateView(changes: SimpleChanges): boolean {
    ...
    if (nzStringTemplateOutlet) {
      if (nzStringTemplateOutlet.firstChange) {
        ...
        const previousOutletType = this.getType(
          nzStringTemplateOutlet.previousValue
        );
        const currentOutletType = this.getType(
          nzStringTemplateOutlet.currentValue
        );
    ...
    const shouldContextRecreate =
      ...
      this.hasContextShapeChanged(nzStringTemplateOutletContext);
  ...
  private updateExistingContext(ctx: any): void {
    for (const propName of Object.keys(ctx)) {
      ...
      (this.inputViewRef!.context as any)[
        propName
      ] = this.nzStringTemplateOutletContext[propName];
    }
  }
